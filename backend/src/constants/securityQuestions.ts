...
export const SECURITY_QUESTION_LABELS: Record<SecurityQuestionKey, string> =
  SECURITY_QUESTIONS.reduce(
    (map, question) => ({
      ...map,
      [question.key]: question.label,
    }),
    {} as Record<SecurityQuestionKey, string>
  );

export const isSecurityQuestionKey = (
  value: string
): value is SecurityQuestionKey =>
  SECURITY_QUESTIONS.some((question) => question.key === value);
